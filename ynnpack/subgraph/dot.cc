...
bool can_convert_f32_to_bf16(ynn_subgraph_t subgraph, uint32_t input_a_id,
                             uint32_t input_b_id) {
  /*return (subgraph->flags & ~YNN_FLAG_CONSISTENT_ARITHMETIC) &&
         (subgraph->flags & YNN_FLAG_DOT_BF16) &&
         subgraph->value(input_a_id).type == ynn_type_fp32 &&
         subgraph->value(input_b_id).type == ynn_type_fp32;*/
  return subgraph->value(input_a_id).type == ynn_type_fp32 &&
         subgraph->value(input_b_id).type == ynn_type_fp32;
}
...
ynn_status define_bf16_dot_from_f32_inputs(
  ...
  status = define_dot(subgraph, num_k_dims, a_bf16, b_residual,
                      YNN_INVALID_VALUE_ID, &a_bf16_b_residual_dot, flags);
  if (status != ynn_status_success) return status;
  ...
  uint32_t residual_sum = YNN_INVALID_VALUE_ID;
  status = ynn_define_binary(subgraph, ynn_binary_add, a_residual_b_bf16_dot,
                             a_bf16_b_residual_dot, &residual_sum, flags);
  if (status != ynn_status_success) return status;

  return ynn_define_binary(subgraph, ynn_binary_add, residual_sum,
                           a_bf16_b_bf16_dot, output_id, flags);
}
...
ynn_status ynn_define_dot(ynn_subgraph_t subgraph, size_t num_k_dims,
  ...
  if (can_convert_f32_to_bf16(subgraph, input_a_id, input_b_id)) {
